...
public class SessionManager {
  ...
  private static final int[] DHT_GET_PEERS_REPLY_ALERT_TYPES = {
  ...
  public void start(SessionParams params) {
    ...
    try {
      ...
      params.settings()
          .setInteger(settings_pack.int_types.alert_mask.swigValue(), alertMask(logging).to_int());
  ...
  /**
   * This method blocks during the destruction of the native session, it could take some time, don't
  ...
  /**
   * This method blocks for at least a second plus the time needed to destroy the native session,
   * don't call it from the UI thread.
   */
  public void restart() {
    sync.lock();
  ...
  /**
   * This function will post a {@link SessionStatsAlert} object, containing a snapshot of the
   * performance counters from the internals of libtorrent.
   */
  ...
  /**
   * This functions instructs the session to post the {@link com.frostwire.jlibtorrent.alerts.StateUpdateAlert},
   * containing the status of all torrents whose state changed since the last time this function was
   * called.
   * <p>
  ...
  public TorrentHandle addTorrent(TorrentInfo torrentInfo, File saveDirectory) {
    ...
    parameters.set_ti(torrentInfo.swig());
    parameters.setSave_path(saveDirectory.getAbsolutePath());
      ...
      if (priorities != null) {
        if (ti.numFiles() != priorities.length) {
          throw new IllegalArgumentException(
              "priorities count should be equals to the number of files");
        }
  ...
  /**
   * Similar to call {@link #fetchMagnet(String, int, boolean, int)} with a maximum size of 2MB.
   *
   * @param uri     magnet uri
   * @param timeout in seconds
   * @param extra   if extra data is included
  ...
  /**
   * Similar to call {@link #fetchMagnet(String, int, boolean)} with a maximum {@code extra =
   * false}.
   *
   * @param uri
   * @param timeout
   ...
   * <p>
   * It's useful in the case your platform doesn't support the built in IP notifier mechanism, or if
   * you have a better more reliable way to detect changes in the IP routing table.
   */
